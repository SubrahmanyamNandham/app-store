...
class AppStore extends Component {
  ...
  getFilterData = () => {
    ...
    const filteredDataList = appsList.filter(each =>
  ...
  render() {
    ...
    const filteredDataList = this.getFilterData()
    const filteredApps = this.getActiveTabApps(filteredDataList)
        ...
        <div className="app-store">
          ...
          <ul className="apps-list">
            {filteredApps.map(each => (
              <AppItem appDetails={each} key={each.id} />
            ))}
          </ul>
        </div>
      </div>
